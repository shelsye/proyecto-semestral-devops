# Actualiza README para Evaluación Final DevOps
...
├── back-Despachos\_SpringBoot/        # Microservicio Despachos (Spring Boot + Dockerfile)
├── back-Ventas\_SpringBoot/           # Microservicio Ventas (Spring Boot + Dockerfile)
├── front\_despacho/                   # Frontend React + Nginx (Dockerfile + nginx.conf)
...
cp .env.example .env          # ajusta MYSQL\_ROOT\_PASSWORD si quieres
export AWS\_REGION=us-east-1
export MYSQL\_ROOT\_PASSWORD=admin123
```

### 5.2 Crear el clúster EKS

1. Obtén el ID de tu cuenta y reemplázalo en `infra/eksctl-cluster.yaml` (campo `<ACCOUNT\_ID>`):
...
```bash
IMAGE\_TAG=latest ./scripts/build-and-push.sh
```

### 5.6 Desplegar en EKS

```bash
IMAGE\_TAG=latest ./scripts/deploy.sh
```
...
|Secret|Valor|
|-|-|
|`AWS\_ACCESS\_KEY\_ID`|de AWS Academy|
|`AWS\_SECRET\_ACCESS\_KEY`|de AWS Academy|
|`AWS\_SESSION\_TOKEN`|de AWS Academy (¡se renueva cada sesión!)|
|`MYSQL\_ROOT\_PASSWORD`|la contraseña de la BD|
...
* **El Secret no se versiona:** `k8s/02-mysql-secret.yaml` es solo una **plantilla** con el placeholder `${MYSQL\_ROOT\_PASSWORD}`; el valor real se inyecta en el despliegue (envsubst en local, o `kubectl create secret` desde un GitHub Secret en CI).
...
|Pods en `ImagePullBackOff`|El rol de los nodos no puede leer de ECR. En Learner Lab usa `LabRole` (ya incluido). *Fallback:* `kubectl -n innovatech create secret docker-registry ecr-registry-key --docker-server=$ECR\_REGISTRY --docker-username=AWS --docker-password=$(aws ecr get-login-password)` y referenciarlo con `imagePullSecrets`.|
